...
cursor.execute('CREATE TABLE servicos(id_servicos INT AUTO_INCREMENT PRIMARY KEY,nome VARCHAR(20) NOT NULL UNIQUE,preco VARCHAR(50) NOT NULL,horario VARCHAR(4) NOT NULL,id_empresa INT,FOREIGN KEY(id_empresa) REFERENCES empresa(id_empresa))')
cursor.execute('CREATE TABLE pedido(id_pedido INT AUTO_INCREMENT PRIMARY KEY,data DATE NOT NULL,horarioinicio VARCHAR(4) NOT NULL,id_clientes INT NOT NULL, id_empresa INT NOT NULL,FOREIGN KEY(id_clientes) REFERENCES clientes(id_clientes),FOREIGN KEY(id_empresa) REFERENCES empresa(id_empresa))')
cursor.execute('CREATE TABLE detalhes(id_pedido INT NOT NULL, id_servicos INT NOT NULL, PRIMARY KEY(id_pedido, id_servicos), FOREIGN KEY(id_pedido) REFERENCES pedido(id_pedido),FOREIGN KEY(id_servicos) REFERENCES servicos(id_servicos))')
